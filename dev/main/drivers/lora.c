...
esp_err_t lora_send_ack(uint8_t dest_id, uint8_t sequence)
{
    ack_payload_t ack = {
        ...
        .snr = (int8_t)(status.last_snr * 4),  // Convert to 0.25 dB units
        .suggested_power = lora_get_suggested_power()
    };
    
    return lora_send_secure(dest_id, MSG_TYPE_ACK,
                            (const uint8_t *)&ack, sizeof(ack_payload_t));
}

esp_err_t lora_send_ping(uint8_t dest_id, bool locate)
{
    // Ping serves dual purpose: simple ping or locate request
    // Locate flag is passed in payload if needed
    ping_payload_t ping = {
        .ping_type = locate ? 1 : 0,
        .reserved = {0, 0, 0}
    };
    return lora_send_secure(dest_id, MSG_TYPE_PING, 
                            (const uint8_t *)&ping, sizeof(ping_payload_t));
}
    ...
    switch (msg_type) {
        case MSG_TYPE_WEATHER_DATA:
            if (len >= sizeof(weather_payload_t) && weather_callback) {
                weather_callback(src_id, (const weather_payload_t *)payload, status.last_rssi);
                // Auto-send ACK with link quality
                lora_send_ack(src_id, tx_sequence - 1);
            }
            break;
        ...
        case MSG_TYPE_PING:
            // Check if this is a locate request
            if (len >= sizeof(ping_payload_t)) {
                const ping_payload_t *ping = (const ping_payload_t *)payload;
                if (ping->ping_type == 1) {
                    // Locate request - trigger LED flash etc
                    ESP_LOGI(TAG, "LOCATE received - should trigger LED flash");
                    // TODO: Add callback for locate behavior
                }
            }
            // Respond with PONG
            {
                pong_payload_t pong = {
                    .ping_type = (len >= sizeof(ping_payload_t)) ? 
                                 ((ping_payload_t *)payload)->ping_type : 0,
                    .rssi = status.last_rssi,
        ...
        case MSG_TYPE_PONG:
            ...
            ESP_LOGI(TAG, "PONG from 0x%02X (RSSI=%d)", src_id, status.last_rssi);
            break;
